...
import 'package:lifestep/tools/components/form/select/selectable/logic/state.dart';
...
class SelectableSelectCubit extends Cubit<SelectableSelectState> {
  ...
  setValue(String? value, index) {
    ...
    if(dataMap != null) {
      value = value;
      changed = this.initialValue != value ? true : false;
  ...
  applyValue() async{
    if(state.changed && state.value != null && state.value != '' && validateFunction(state.value, dataMap)){
      emit(SelectableSelectState(confirmedValue: confirmedValue, display: state.display, value: state.value, changed: this.value != initialValue ? true : false));
