...
class PreviewPolylineMapCubit extends Cubit<PreviewPolylineMapState> {
  ...
  Future<void> initialize() async {
    polylines = {};
    if(challengeModel.mapJsonGet().length > 1) {
      List<LatLng> coordinates = challengeModel.mapJsonGet().map((element) =>
          LatLng(element[1], element[0])).toList();
      _addPolyLine(polykey: "poly-key-${0.toString()}",
          startLatLng: LatLng(
